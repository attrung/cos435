#!/bin/bash
# NFSP-IQN mean-variance — 64x64 architecture, 50M episodes, snapshot every 10M.
# var_penalty=0.5 (matches NOTES_FOR_PAPER's iqn_mv05 setting).
set -euo pipefail
cd "$(dirname "$0")"

NAME=holdem_64x64_meanvar
HIDDEN="64,64"
WORKERS=3
WORKER_BATCH=4
EPISODES=50000000
...
echo "============================================================"
echo "  NFSP-IQN C++ Hold'em — MEAN-VARIANCE 64x64 (50M)"
echo "  Arch: [${HIDDEN}], Adam lr=${DQN_LR}/${AVG_LR}, IQN N=${IQN_N}, var_penalty=${VAR_PENALTY}"
